...
static void ShowExampleAppCustomRendering(bool* p_open)
{
    ...
    ImGui::Separator();
    {
        ...
        for (int i = 0; i < points.Size - 1; i += 2)
            draw_list->AddRect(ImVec2(canvas_pos.x + points[i].x, canvas_pos.y + points[i].y), ImVec2(canvas_pos.x + points[i+1].x, canvas_pos.y + points[i+1].y), IM_COL32(255,255,0,255), 2.0f);
        draw_list->PopClipRect();
        if (adding_preview)
            points.pop_back();
    }
